...
export default function Header() {
  ...
  const isStartActive = location.pathname === '/start-a-project';

  return (
    <header
      className={`fixed top-0 left-0 right-0 z-40 transition-all duration-300 ${
        isScrolled
          ? 'bg-obsidian-950/90 backdrop-blur-md border-b border-white/[0.08] py-3 shadow-xl shadow-black/40'
          : 'bg-gradient-to-b from-black/80 via-black/30 to-transparent py-4 sm:py-5'
      }`}
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-11">
          ...
          {/* Desktop Navigation & Actions: Perfectly aligned on same flex baseline */}
          <div className="hidden lg:flex items-center gap-6 xl:gap-8">
            {/* Nav Menu Items with uniform typography, sizing, and letter-spacing */}
              ...
              {navLinks.map((item) => {
                ...
                return (
                  <Link
                    key={item.name}
                    to={item.href}
                    className={`inline-flex items-center text-[12px] tracking-[0.14em] uppercase font-medium transition-colors duration-200 relative h-9 px-1 ${
                      isActive
            ...
            {/* Right Action Buttons: Consistent height (h-9), uniform letter spacing, and balanced spacing */}
            <div className="flex items-center gap-3 shrink-0">
              {/* Contact Button */}
              <Link
                to="/contact"
                className={`h-9 px-4 rounded-full border text-[12px] font-medium tracking-[0.14em] uppercase transition-all duration-200 inline-flex items-center justify-center ${
                  isContactActive
              ...
              <Link
                to="/start-a-project"
                className="h-9 px-5 rounded-full bg-gradient-to-r from-luxe-gold to-luxe-bronze text-obsidian-950 text-[12px] font-semibold tracking-[0.14em] uppercase hover:brightness-105 active:scale-[0.98] transition-all duration-200 inline-flex items-center justify-center gap-1.5 shadow-sm shadow-black/30"
              >
          ...
          <div className="flex items-center gap-2.5 lg:hidden">
            <Link
              to="/contact"
              className={`h-8 px-3 rounded-full border text-[10.5px] font-medium tracking-[0.12em] uppercase transition-all inline-flex items-center justify-center ${
                isContactActive
                  ? 'border-luxe-gold text-luxe-gold bg-luxe-gold/10'
            ...
            <Link
              to="/start-a-project"
              className="h-8 px-3.5 rounded-full bg-luxe-gold text-obsidian-950 text-[10.5px] font-semibold tracking-[0.12em] uppercase inline-flex items-center justify-center shadow-sm"
            >
              Start
            </Link>
            <button
              onClick={() => setMobileMenuOpen(!mobileMenuOpen)}
              className="h-8 w-8 rounded-lg bg-obsidian-900/90 border border-white/15 text-neutral-200 hover:text-white flex items-center justify-center focus:outline-none ml-1"
      ...
      {/* Mobile Drawer Menu */}
      {mobileMenuOpen && (
        <div className="lg:hidden fixed inset-x-0 top-[57px] bg-obsidian-950/98 backdrop-blur-2xl border-b border-white/10 shadow-2xl px-6 py-7 min-h-[calc(100vh-57px)] flex flex-col justify-between overflow-y-auto animate-fade-in">
